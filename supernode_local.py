...
def remove_server_from_leader_config():
    try:
        ...
        if MODE != 'LOCAL':
            ...
            file_path = os.path.expanduser(f"~/leader_config.txt")
            ...
            if current_leader in file_content:
                ...
                # Update the local leader_config.txt file with the modified content
...
def read_leader_config():
    if MODE != 'LOCAL':
        ...
        file_path = os.path.expanduser("~/leader_config.txt")
        try:
            with open(file_path, 'r') as file:
                content = file.read()
...
def write_leader_config(data):
    if MODE != 'LOCAL':
        ...
        with open(file_path, 'w') as file:
            file.write(data)
        print(f"Data written to the local file: {file_path}")
...
def create_alive_file(node_id):
    ...
    if MODE != 'LOCAL':
        ...
        file_name = f'~/node_{node_id}.alive'
        file_key = os.path.expanduser(file_name)  # File path in the user's home directory

        if not os.path.exists(file_key):
            with open(file_key, 'w') as file:
                file.write("This file indicates that the server is alive.")
            print(f"Alive file created at {file_key}")
        else:
            print(f"Alive file already exists at {file_key}")
...
def remove_alive_file():
    ...
    if MODE != 'LOCAL':
        ...
        file_path = os.path.expanduser(f"~/node_{node_id}.alive")
        # Check if the file exists locally before attempting to delete
        if os.path.exists(file_path):
            os.remove(file_path)
            print(f"Alive file for node {node_id} removed locally at {file_path}")
        else:
            print(f"Alive file for node {node_id} not found locally")
    ...
    while True:
        ...
        if current_leader is None or not does_s3_object_exist(BUCKET_NAME, s3_key):
            ...
            if MODE!='LOCAL':
                ...
                available_nodes = [ip_to_node_id[node['ip']] for node in nodes if
                                   os.path.exists(os.path.expanduser(f'~/node_{ip_to_node_id[node["ip"]]}.alive'))]
